...
test("getInterviewersForDay returns an array containing the correct interviewer object", () => {
  const interviewerArray = getInterviewersForDay(state, "Tuesday");
  const interviewer = interviewerArray[0];
  expect(interviewer.name).toEqual("Tori Malcolm");
});

test("getInterviewersForDay returns an empty array when the days data is empty", () => {
  const result = getInterviewersForDay({ days: [] }, "Sunday");
  expect(result.length).toEqual(0);
});
